    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-slate-700 shadow-sm transition-all placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-500/10"
          value={searchTerm}
          ...
          <table className="w-full text-left text-sm text-gray-500">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredPatients.map(patient => (
                <tr key={patient.id} className="group hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <button onClick={() => setSelectedProfile(patient)} className="font-semibold text-gray-900 hover:text-teal-600 hover:underline">
                      {patient.name}
                    </button>
                  </td>
                  <td className="px-6 py-4">{patient.age} Y / {patient.sex}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-3">
                      ...
                      {/* Edit Button now opens the full profile */}
                      <button 
                      ...
                      >
                        <Edit2 size={18} /> <span className="text-xs font-bold">Edit</span>
                      </button>
              ...
              {filteredPatients.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-500">No patients found.</td>
                </tr>
              )}
            </tbody>
          </table>
